...
function () {
  function Customizator() {
    ...
    this.colorPicker = document.createElement('input'); //! элемент чтобы юзер смог сбросить настройки

    this.clear = document.createElement('div'); //! получаем из localStorage масштаб и цвет, || пишем чтобы было значение по умолчанию 

    this.scale = localStorage.getItem('scale') || 1;
    this.color = localStorage.getItem('color') || '#ffffff'; // вешаем обработчик('при клике', (объект событие) => метод onScaleChange(объект событие))
  ...
  _createClass(Customizator, [{
    ...
    value: function onScaleChange(e) {
      ...
      var body = document.querySelector('body'); // получ доступ к всем элементам на стр
        ...
        element.childNodes.forEach(function (node) {
          ...
          if (node.nodeName === '#text' && node.nodeValue.replace(/\s+/g, '').length > 0) {
            if (!node.parentNode.getAttribute('data-fz')) {
              // если атрибута 'data-fz' не существует
              //* получаем размер шрифта.  
              var value = window.getComputedStyle(node.parentNode, null).fontSize; // getComputedStyle() позволяет получить все стили кот были применены к элементу
              //* нода.ее родитель.добавитьАтрибут('data-fz', тут value к числу и регулярн выраж убирающее px)

              node.parentNode.setAttribute('data-fz', +value.replace(/px/g, '')); //! нода.ее родитель.стили.фонтСайз = нода.ее родитель.получитьАтрибут('data-fz') * масштаб + пиксели
            ...
            // если условие не выполн. то мы запускаем функ еще раз 
            recursy(node); // node - каждый отдельный элемент внутри массива element.childNodes
          }
        });
      };

      recursy(body); // записылваем this.scale в локалстораж
    ...
    value: function render() {
      ...
      this.onScaleChange(); // создаем элеметны чтобы разместить их на стр

      var scaleInputS = document.createElement('input'),
          scaleInputM = document.createElement('input'),
          panel = document.createElement('div'); //! append вставляет эл-ты внутрь родительского элемента, в конец
      ...
      this.colorPicker.classList.add('color'); //! меняем type у инпутов
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var panel = new _customizator__WEBPACK_IMPORTED_MODULE_0__["default"](); //! в panel лежит объект Customizator из файла customizator.js
